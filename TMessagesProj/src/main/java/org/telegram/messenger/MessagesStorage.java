...
public class MessagesStorage extends BaseController {
    ...
    public static void createTables(SQLiteDatabase database) throws SQLiteException {
        ...
        database.executeFast("CREATE TABLE dialog_filter_octo(id INTEGER PRIMARY KEY, ord INTEGER, unread_count INTEGER, flags INTEGER, title TEXT, emoticon TEXT, color INTEGER DEFAULT -1, entities BLOB, noanimate INTEGER)").stepThis().dispose();
    ...
    private void loadDialogFilters() {
        storageQueue.postRunnable(() -> {
            ...
            try {
                ...
                filtersCursor = database.queryFinalized("SELECT id, ord, unread_count, flags, title, emoticon, color, entities, noanimate FROM dialog_filter_octo WHERE 1");
