 ...
 */
package it.octogram.android.camera;
...
public class CameraXUtils {
    ...
    public static void loadSuggestedResolution() {
        ...
        getAvailableVideoSizes().values().stream()
                ...
                .ifPresent(height -> {
                    ...
                    if (OctoConfig.cameraResolution == -1 || OctoConfig.cameraResolution > max || OctoConfig.cameraResolution < min) {
                        OctoConfig.saveCameraResolution(height);
                    }
                });
    }
    ...
    public static Size getPreviewBestSize() {
        ...
        return getAvailableVideoSizes().values().stream()
                .filter(size -> size.getHeight() <= OctoConfig.cameraResolution && size.getHeight() <= suggestedRes)
    ...
    public static Quality getVideoQuality() {
        return getAvailableVideoSizes().entrySet().stream()
                .filter(entry -> entry.getValue().getHeight() == OctoConfig.cameraResolution)
    ...
    private static int getSuggestedResolution(boolean isPreview) {
        ...
        switch (SharedConfig.getDevicePerformanceClass()) {
            ...
            default:
                suggestedRes = OctoConfig.useCameraXOptimizedMode && isPreview ? 1080 : 2160;
                break;
        }
        return suggestedRes;
    }
