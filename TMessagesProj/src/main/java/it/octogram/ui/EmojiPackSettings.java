...
import it.octogram.ui.Cells.EmojiSet;
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        ...
        if (position >= emojiPacksStartRow && position < emojiPacksEndRow) {
            ...
            if (CustomEmojiController.emojiDir(cell.packId, cell.versionWithMD5).exists() || cell.packId.equals("default")) {
                OctoConfig.setEmojiPackSelected(cell.packId);
                cell.setChecked(true, true);
                Emoji.reloadEmoji();
                NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.emojiLoaded);
                if (OctoConfig.useSystemEmoji) {
                    OctoConfig.toggleUseSystemEmoji();
            ...
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(OctoConfig.useSystemEmoji);
            }
            Emoji.reloadEmoji();
            NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.emojiLoaded);
            if (OctoConfig.useSystemEmoji) {
            ...
            switch (ViewType.fromInt(holder.getItemViewType())) {
                ...
                case EMOJI_PACK_SET_CELL:
                    ...
                    if (emojiPackInfo != null) {
                        emojiPackSetCell.setChecked(!hasSelected() && emojiPackInfo.getPackId().equals(CustomEmojiController.getSelectedEmojiPackId()) && getCurrentDownloading() == null && getCurrentUnzipping() == null && !OctoConfig.useSystemEmoji, partial);
                ...
                case SWITCH:
                    ...
                    if (position == useSystemEmojiRow) {
                        if (!partial) textCheckCell.setTextAndValueAndCheck(LocaleController.getString("UseSystemEmojis", R.string.UseSystemEmojis), LocaleController.getString("UseSystemEmojisDesc", R.string.UseSystemEmojisDesc), OctoConfig.useSystemEmoji, true, false);
                        if (partial) textCheckCell.setChecked(OctoConfig.useSystemEmoji);
                    }
                    break;
